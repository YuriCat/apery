...
import model.pv170526_bn as mdl
...
test_time_periods = datetime.timedelta(seconds=60 * 120)# テストを行う時間頻度
reconstruct_num = 2 ** 17
test_num = 2 ** 17

model_file_name = "pv170526_bn"

def learn(mdl_file,
          train_hcpe_data_path, train_hcpr_data_path,
          test_hcpe_data_path, test_hcpr_data_path,
          data_batch_size, batch_size, mepoch):
    ...
    with tf.Graph().as_default():
        ...
        for e in range(mepoch):
            if datetime.datetime.today() - tstart >= test_time_periods: # 最初は必ずテストを呼ぶ
                ...
                for ph in range(N_PHASES):
                    ...
                    for j in range(0, reconstruct_num, data_batch_size):
                        inputs, moves, move_labels, values = mdl.load_inputs_moves_labels_values(train_hcpe_data_path, data_batch_size)
                    ...
                    print("train(hcpe) - move: loss %g acc %g value: loss %g mae %g  (/%d)" % (test_result_sum[0], test_result_sum[1],
                ...
                for ph in range(N_PHASES):
                    ...
                    for j in range(0, test_num, data_batch_size):
                        inputs, moves, move_labels, values = mdl.load_inputs_moves_labels_values(test_hcpe_data_path, data_batch_size)
                        for i in range(0, data_batch_size, batch_size):
                            test_result_sum += sess.run(test_op[ph],
                                                        feed_dict={
                                                            x_placeholder: inputs[i:(i + batch_size)],
                    ...
                    print("test(hcpe)  - move: loss %g acc %g value: loss %g mae %g  (/%d)" % (test_result_sum[0], test_result_sum[1],
                                                                                         test_result_sum[2], test_result_sum[3],
                                                                                         test_num))
                # reconstruct phase
                print("[%d policy samples, %d value samples] %s" % (policy_learned_num, value_learned_num,
                ...
                for ph in range(N_PHASES):
                    test_result_sum = np.zeros(4)
                    for j in range(0, reconstruct_num, data_batch_size):
                        inputs, moves, move_labels, values = mdl.load_inputs_moves_labels_values(train_hcpr_data_path, data_batch_size)
                        for i in range(0, data_batch_size, batch_size):
                            test_result_sum += sess.run(test_op[ph],
                                                        feed_dict={
                                                            x_placeholder: inputs[i:(i + batch_size)],
                                                            ...
                                                            is_training_placeholder: False})
                    test_result_sum /= reconstruct_num
                    print("train(record) - move: loss %g acc %g value: loss %g mae %g  (/%d)" % (test_result_sum[0], test_result_sum[1],
                                                                                                 test_result_sum[2],test_result_sum[3],
                                                                                                 reconstruct_num))
                ...
                for ph in range(N_PHASES):
                    test_result_sum = np.zeros(4)
                    for j in range(0, test_num, data_batch_size):
                        inputs, moves, move_labels, values = mdl.load_inputs_moves_labels_values(test_hcpr_data_path, data_batch_size)
                        for i in range(0, data_batch_size, batch_size):
                            test_result_sum += sess.run(test_op[ph],
                                                        feed_dict={
                                                            x_placeholder: inputs[i:(i + batch_size)],
                                                            ...
                                                            is_training_placeholder: False})
                    test_result_sum /= test_num
                    print("test(record)  - move: loss %g acc %g value: loss %g mae %g  (/%d)" % (test_result_sum[0], test_result_sum[1],
                                                                                                 test_result_sum[2], test_result_sum[3],
                                                                                                 test_num))
            ...
            # trainig phase (hcpe から policy - value 同時)
            for ph in range(N_PHASES):
                inputs_e, moves_e, move_labels_e, values_e = mdl.load_inputs_moves_labels_values(train_hcpe_data_path,
                                                                                                 data_batch_size)
                inputs_r, moves_r, move_labels_r, values_r = mdl.load_inputs_moves_labels_values(train_hcpr_data_path,
                                                                                         data_batch_size)
                for i in range(0, data_batch_size, batch_size):
                    sess.run(train_op[ph],
                             feed_dict = {
                                 x_placeholder: inputs_e[i:(i + batch_size)],
                                 move_placeholder: moves_e[i:(i + batch_size)],
                                 value_placeholder: values_e[i:(i + batch_size)],
                                 is_training_placeholder: True,
                                 learning_rate_placeholder: learning_rate})
                    if i % (batch_size * 8) == 0:
                        sess.run(move_train_op[ph],
                                 feed_dict = {
                                     x_placeholder: inputs_r[i:(i + batch_size)],
                                     move_placeholder: moves_r[i:(i + batch_size)],
                                     is_training_placeholder: True,
                                     learning_rate_placeholder: learning_rate})
                ...
                # 学習局面数更新
                policy_learned_num += data_batch_size // 8
...
if __name__ == "__main__":
    ...
    train_hcpe_data_path = sys.argv[3]
    train_hcpr_data_path = sys.argv[4]
    test_hcpe_data_path = sys.argv[5]
    test_hcpr_data_path = sys.argv[6]
    mdl_file = sys.argv[7]
    mepoch = 1000000000
